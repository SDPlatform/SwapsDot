...
contract SDFeeTo {
    ...
    function setOwner(address owner_) public {
        require(msg.sender == owner, 'SDFeeTo::setOwner: not allowed');
        owner = owner_;
    }

    function setFeeRecipient(address feeRecipient_) public {
        require(msg.sender == owner, 'SDFeeTo::setFeeRecipient: not allowed');
        feeRecipient = feeRecipient_;
    }

    function updateTokenAllowState(address token, bool allowed) public {
        require(msg.sender == owner, 'SDFeeTo::updateTokenAllowState: not allowed');
    ...
    function renounce(address pair) public returns (uint value) {
        ...
        TokenAllowState storage token0AllowState = tokenAllowStates[ISDswapV2Pair(pair).token0()];
        TokenAllowState storage token1AllowState = tokenAllowStates[ISDswapV2Pair(pair).token1()];
        ...
        if (
            ...
            value = ISDswapV2Pair(pair).balanceOf(address(this));
            if (value > 0) {
                ...
                assert(ISDswapV2Pair(pair).transfer(pair, value));
                ISDswapV2Pair(pair).burn(pair);
            }
    ...
    function claim(address pair) public returns (uint value) {
        ...
        if (
            ...
            if (value > 0) {
                ...
                assert(ISDswapV2Pair(pair).transfer(feeRecipient, value));
            }
        }
    }
}

interface ISDswapV2Pair {
